...
module.exports = {
    ...
    view: async function (req, res) {
        ...
        var model = await Person.findOne(req.params.id).populate('includes');

        var model1 = await Person.findOne(req.params.id).populate('includes',{id:req.session.userid});

        console.log("model1", model1);

        if (!model) return res.notFound();

        sails.log("view info: ", req.body);

        return res.view('person/view', { person: model, userrent: model1});
    ...
    populate: async function (req, res) {
        ...
        var person = await Person.findOne(req.params.id).populate('includes');
    
        if (!person) return res.notFound();
    
        return res.view('person/rent', { person: person });
